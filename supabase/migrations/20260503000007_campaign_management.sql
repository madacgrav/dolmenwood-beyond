...
-- 6. join_campaign RPC: player calls this with an invite code to join a campaign.
--    Returns the campaign_id on success, NULL if the code is invalid.
--    Runs as security definer so it can SELECT campaigns without a broad policy.
create or replace function public.join_campaign(p_invite_code text)
returns uuid
language plpgsql
security definer
